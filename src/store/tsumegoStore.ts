...
interface TsumegoState {
    // Library Data
    volumes: TsumegoVolume[];
    flatProblems: TsumegoProblem[];
...
const getSetupStones = (node: SGFNode): Point[] => {
    ...
    console.log("Setup Stones:", stones.length, stones);
    return stones;
};

export const useTsumegoStore = create<TsumegoState>()(
    persist(
        (set, get) => ({
            volumes: [],
            flatProblems: [],
            ...
            loadLibrary: async () => {
                ...
                try {
                    const volumes = await loadTsumegoVolumes();
                    const flatProblems: TsumegoProblem[] = [];
                    let globalIndex = 1;

                    volumes.forEach(v => {
                        v.chapters.forEach(c => {
                            c.problems.forEach(p => {
                                p.label = `死活第 ${globalIndex++} 题`;
                                p.group = v.title;
                                flatProblems.push(p);
                            });
                        });
                    ...
                    console.log("Tsumego Library loaded:", flatProblems.length, "problems");
                    set({ volumes, flatProblems, isLoadingLibrary: false });
                } catch (e) {
                    console.error("Failed to load tsumego library", e);
                    set({ isLoadingLibrary: false, feedback: "加载死活题库失败" });
                }
            },
            ...
            restoreSession: () => {
                ...
                if (currentProblemId && !sgfRoot && flatProblems.length > 0) {
                    ...
                    if (p) {
                        console.log("Restoring Tsumego Session:", currentProblemId);
            ...
            loadProblem: (sgfContent, problemId) => {
                ...
                // Standardized Format: Title \n Comment (User requested to remove explicit turn indicator as it conflicts with SGF text)
                // const turnStr = blackTurn ? '黑先' : '白先'; 
                const rootComment = root.props.C ? `\n\n${root.props.C[0]}` : '';
                const feedbackText = `${displayTitle}${rootComment}`;
            ...
            showSolution: () => {
                ...
                const step = () => {
                    ...
                    // Heuristic: Find branch with 'Correct' or 'Tesuji' message
